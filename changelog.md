## Changelog

### TODO
- Fix double-click on empty desktop space not triggering
- Add support for corner hotkeys
- Add double-click system tray to toggle disable/enable desktop show/hide icons

### Known Limitations
- **Double-click toggle disabled**: The desktop double-click to toggle icons feature has compatibility issues with Windows hooks in .NET applications. Use the hotkey (default: Ctrl+Alt+D) or system tray menu instead.

### 1.3.4
- Added support for hiding corner countdowns per corner
- Added "Show Timer" checkbox for each corner in settings

### 1.3.3
- Added visual countdown timer when hovering over a hot corner

### 1.3.2
- Added version number display to settings window
- Added global hotkey to toggle hot corners on/off (default: Ctrl+Alt+H)
- Users can now customize the toggle hotkey with any key combination

### 1.3
- Added "apply" button to settings window
- Added individual corner timing support (each corner can have its own activation delay)

### 1.2
- Added Desktop icons show/hide functionality
- Added global hotkey support (Ctrl+Alt+D to toggle desktop icons)
- Added system tray integration

### 1.1
- Fixes

### 1.0
- Initial release
